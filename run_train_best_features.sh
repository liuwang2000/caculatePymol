...
# 标题
echo -e "\e[1m使用train_rf_model配置进行最优特征选择\e[0m"
echo "==============================="
echo
...
# 检查train_and_evaluate_best_features.py是否存在
if [ ! -f "train_and_evaluate_best_features.py" ]; then
    echo -e "${RED}错误: 找不到train_and_evaluate_best_features.py文件${NC}"
    exit 1
fi
...
DEFAULT_OUTPUT_DIR="./rf_feature_models"
DEFAULT_MIN_FEATURES=3
DEFAULT_MAX_FEATURES=15
DEFAULT_MAX_COMBINATIONS=5000
...
# 询问最大组合数
echo -e "${BLUE}请输入最多测试的组合数量 (默认: ${DEFAULT_MAX_COMBINATIONS}, 0表示测试所有组合):${NC}"
read max_combinations
...
echo -e "${BLUE}最多组合数:${NC} $max_combinations"
...
# 运行优化
echo -e "${GREEN}开始执行特征评估...${NC}"
echo -e "${YELLOW}这可能需要几分钟到几小时时间，取决于特征组合数量...${NC}"

# 构建命令
cmd="python train_and_evaluate_best_features.py --data \"$data_file\" --output \"$output_dir\" --min_features $min_features --max_features $max_features --n_jobs $THREADS $include_aa_flag"

if [ $max_combinations -gt 0 ]; then
    cmd="$cmd --max_combinations $max_combinations"
fi
...
if [ $? -eq 0 ]; then
    echo -e "${GREEN}特征评估完成!${NC}"
    echo -e "${GREEN}评估结果保存在: $output_dir${NC}"
    echo -e "${GREEN}模型选择标准: 测试集R²最高的模型已被保存${NC}"
else
    echo -e "${RED}评估过程出错。请检查日志以获取详细信息。${NC}"
fi

echo
echo -e "\e[32m评估完成！\e[0m"
